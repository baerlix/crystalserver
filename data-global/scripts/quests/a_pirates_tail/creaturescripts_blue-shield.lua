...
function ratmiralCheeseClick.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if toPosition.x ~= cheeseConfig.position.x or
       toPosition.y ~= cheeseConfig.position.y or
       toPosition.z ~= cheeseConfig.position.z then
        return false
    end

    local currentTicks = player:getStorageValue(cheeseConfig.storageKey)
    if currentTicks < 0 then currentTicks = 0 end
